...
v_deb_packages_repo=http://mirror.hetzner.de/ubuntu/packages
v_deb_security_repo=http://mirror.hetzner.de/ubuntu/security
...
zfs create -o com.sun:auto-snapshot=false -o canmount=on -o mountpoint=/tmp rpool/tmp
chmod 1777 /mnt/tmp

echo "======= setting up initial system backages =========="
debootstrap --arch=amd64 bionic /mnt $v_deb_packages_repo
...
#cat > "/mnt/etc/netplan/01-netcfg.yml" <<CONF
#network:
#  version: 2
#  renderer: networkd
#  ethernets:
...
deb [arch=i386,amd64] $v_deb_packages_repo bionic main restricted
deb [arch=i386,amd64] $v_deb_packages_repo bionic-updates main restricted
deb [arch=i386,amd64] $v_deb_packages_repo bionic-backports main restricted
deb [arch=i386,amd64] $v_deb_packages_repo bionic universe
deb [arch=i386,amd64] $v_deb_security_repo bionic-security main restricted
CONF
...
chroot_execute "apt install --yes wget curl software-properties-common nano htop ifupdown"
...
rm -rf /mnt/etc/dropbear-initramfs/dropbear_ecdsa_host_key
...
chroot_execute "zfs set mountpoint=legacy bpool/BOOT/ubuntu"
chroot_execute "echo bpool/BOOT/ubuntu /boot zfs nodev,relatime,x-systemd.requires=zfs-import-bpool.service 0 0 > /etc/fstab"
